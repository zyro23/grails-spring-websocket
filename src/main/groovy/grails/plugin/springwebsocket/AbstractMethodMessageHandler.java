...
public abstract class AbstractMethodMessageHandler<T>
	...
	public void afterPropertiesSet() {
		...
		// System.out.println("Iterating over beans for message mappings");
		Date now = new Date();
		GrailsApplication grailsApplication = (GrailsApplication)(this.applicationContext.getBean("grailsApplication"));

		for (grails.core.GrailsClass artefact : grailsApplication.getArtefacts("Websocket")) {
			String beanName = artefact.getPropertyName();
			detectHandlerMethods(beanName);
		}
		// System.out.println("Completed message mapping in " + (new Date().getTime() - now.getTime()) + " ms");
	}
	...
	private class ReturnValueListenableFutureCallback implements ListenableFutureCallback<Object> {
		...
		public void onSuccess(Object result) {
			try {
				java.lang.reflect.Method getAsyncReturnValueType = this.handlerMethod.getClass().getMethod("getAsyncReturnValueType");
				getAsyncReturnValueType.setAccessible(true);
				MethodParameter returnType  = (MethodParameter) getAsyncReturnValueType.invoke(this.handlerMethod,result);
